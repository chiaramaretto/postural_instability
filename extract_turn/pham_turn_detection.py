...
class PhamTurnDetection:
    ...
    def __init__(
        ...
        min_turn_duration_s: float = 2,
        max_turn_duration_s: float = 8,
        min_turn_angle_deg: float = 60,
    ):
        """
    ...
    def detect(
        ...
        # Select acceleration data and convert it to a writable numpy array.
        accel = accel_data.to_numpy(copy=True)

        # Select gyro data and convert it to a writable numpy array.
        gyro = gyro_data.to_numpy(copy=True)
        self.gyro = gyro
        ...
        # If no stable bias window exists, fall back to zero bias.
        if len(bias_periods) > 0:
            self.gyro_bias = np.mean(gyro[bias_periods, :], axis=0)
        else:
            self.gyro_bias = np.zeros(gyro.shape[1], dtype=gyro.dtype)

        # Subtract gyro bias from the original gyro signal
        gyro_unbiased = gyro ################# commented - self.gyro_bias
