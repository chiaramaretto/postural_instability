...
# --- STEP 1: Clinical Data Representation (DR-SAE) ---
class DR_SAE(nn.Module):
    def __init__(self):
        super().__init__()
        # Ridotto il numero di filtri finali a 32 per un bottleneck clinico
        self.enc_layers = nn.ModuleList([
            nn.Conv1d(1, 16, kernel_size=5, padding=2), # Kernel più grande per basse freq.
            nn.Conv1d(16, 32, kernel_size=3, padding=1),
            nn.Conv1d(32, 64, kernel_size=3, padding=1),
            nn.Conv1d(64, 64, kernel_size=3, padding=1),
            nn.Conv1d(64, 32, kernel_size=3, padding=1)  # Bottleneck finale a 32
        ])
...
# --- STEP 2: Clinical Local Feature Fusion (LFF-AE) ---
class LFF_AE(nn.Module):
    def __init__(self, input_channels=6 * 32, c4_dim=32):
        super().__init__()
        # Encoder: Ridotto drasticamente per isolare marker di instabilità
        ...
        self.dropout = nn.Dropout(0.2) # Aggiunto dropout per robustezza
        self.conv4 = nn.Conv1d(64, c4_dim, kernel_size=3, padding=1) # Target finale: 32 feature
        
        # Decoder (Simmetrico)
        self.deconv4 = nn.ConvTranspose1d(c4_dim, 64, kernel_size=3, padding=1)
        self.deconv3 = nn.ConvTranspose1d(64, 64, kernel_size=3, padding=1)
        ...
        self.deconv1 = nn.ConvTranspose1d(128, input_channels, kernel_size=3, padding=1)
    ...
    def forward(self, x):
        ...
        x = self.dropout(self.selu(self.bn3(self.conv3(x))))
